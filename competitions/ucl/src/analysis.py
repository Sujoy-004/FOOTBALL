...
def run_calibrated_validation(
    ...
    Returns calibrated validation report, or None if calibration config missing.
    Raises ValueError on invalid input.
    """
    from competitions.ucl.src.orchestrator import build_signal_engine, load_calibration

    raw_cal = load_calibration()
    if raw_cal is None or "T" not in raw_cal:
        logger.warning(
            "--validate-calibrated requires calibration config. "
            "Run calibrate-temp first."
        )
        return None
    ...
    if replay_data_path:
        try:
            ...
            logger.warning("Could not load replay data: %s", e)
    ...
    engine.evaluate = _calibrated_evaluate

    try:
        if tier == "all" or tier == "walk-forward":
        ...
        if tier == "all" or tier == "cross-tournament":
        ...
        logger.warning("Calibrated validation failed: %s", e)
