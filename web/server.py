...
def load_json_list(name: str) -> list:
    with open(DATA_DIR / name, encoding="utf-8") as f:
...
def compute_bracket_display(groups, teams, bracket, annex_c, played, played_groups):
    """R32-only bracket for the existing /api/bracket endpoint."""
...
def compute_full_bracket(groups, teams, bracket, annex_c, played, played_groups, ledger=None):
    """Resolve all 6 rounds (R32 → R16 → QF → SF → TPP → FINAL) with teams, scores, winners."""
    elo_ratings = {n: d["elo"] for n, d in teams.items()}
    known_winners = {mid: data["winner"] for mid, data in played.items() if data.get("winner")}
    slot_teams = resolve_knockout_slot_teams(groups, teams, played_groups, bracket, annex_c, known_winners)
    ...
    # Precompute team signal strengths from ledger (for KO signal probabilities)
    if ledger:
    ...
    # — Phase 1: entries resolved by slot_teams (R32 + some R16/QF) —
    ...
    # — Phase 2: remaining entries via source_matches (walk bracket in order) —
    def _resolve_teams_from_source(entry):
        """Return (team_a, team_b) from source match winners (losers for TPP)."""
...
def compute_all():
    ...
    ld = boot_step("Data Loading", lambda: {
        ...
        "groups": load_groups(DATA_DIR, teams=load_json("teams.json")),
    ...
    cov = boot_step("Coverage Audit", lambda: coverage_audit())
    ...
    # Store raw simulation result for what-if baseline comparison
    if sim:
        data["simulation_raw"] = {
...
def compute_signal_stats():
    """Per-signal aggregate stats from ledger (no actual results needed)."""
    data_dir = constants.DATA_DIR
...
def compute_signal_detail(name: str):
    """Drill into one signal: per-match probabilities + eval if available."""
    data_dir = constants.DATA_DIR
    ledger = json.loads((data_dir / "predictions_ledger.json").read_text(encoding="utf-8"))
    played = json.loads((data_dir / "played.json").read_text(encoding="utf-8"))
    ...
    for mid, signals in ledger.items():
        ...
        # Try to find match teams from groups.json or played
        if mid in played:
            m = played[mid]
    ...
    # Also compute backtest metrics for this signal from cache
    cache_file = Path(__file__).parent / "cache.json"
    if cache_file.exists():
...
def compute_blend_info():
    """Current blend weights from backtest + calibration status."""
    data_dir = constants.DATA_DIR
    ...
    # Add uniform fallback for signals not in backtest
    for sk in signals_in_ledger:
        if sk not in briers:
            briers[sk] = 0.25  # random baseline
...
def refresh_from_api():
    """Use BSD_API_KEY to fetch live match results and predictions."""
    global cache
    if not BSD_API_KEY:
    ...
    try:
        ...
        # Process group matches FIRST — updates real results so bracket can be resolved
        ...
        # Resolve bracket slots to actual team names using updated group results
        ...
        # Process knockout matches with resolved (team-based) bracket
...
def api_what_if(req: dict = None):
    ...
    if mode == "instant":
        ...
        # Compute per-signal probabilities for this match
        team_strengths = compute_team_signal_strengths(ledger, played_groups)
        sigs, elo_p = compute_ko_signal_probs(ta, tb, team_strengths, elo_ratings)

        # Build original signals dict with weights (include elo)
        original_signals = {}
        ...
        def _run_sim(task_id, scenario, match_id, ta, tb, iterations):
            try:
                ...
                # Apply scenario adjustment to blend params
                blend_info = compute_blend_info()
                blend_weights = blend_info.get("blend_weights", {})
                team_strengths = compute_team_signal_strengths(ledger, played_groups)
                elo_ratings = {n: d["elo"] for n, d in teams_raw.items()}
                ...
                # Build xg_overrides from scenario adjustments
                xg_overrides = None
                ...
                with sim_lock:
                    active_simulations[task_id]["status"] = "running"
                    active_simulations[task_id]["progress"] = 50.0
...
@app.get("/api/simulation/progress/{task_id}")
