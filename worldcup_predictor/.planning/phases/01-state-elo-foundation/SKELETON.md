...
┌─────────────────────────────────────────────────────┐
│                   main.py                            │
│         (entry point — loads state, prints           │
│          summary; CLI commands in later phase)        │
└────────────┬──────────────────────────┬──────────────┘
             │                          │
    ┌────────▼────────┐       ┌─────────▼─────────┐
...
│   └── test_integration.py  # End-to-end Elo + persistence roundtrip
├── main.py                  # Entry point
...
1. **Startup:** `main.py` calls `state.load_teams()`, `state.load_bracket()`, `state.load_played()` and prints summary
2. **Validation:** `state.validate_bracket()` runs inside `load_bracket()` — rejects bad bracket data
3. **Persistence:** `state.save_teams()` and `state.save_played()` atomically write JSON files using tempfile + os.replace + os.fsync
4. **Integration:** `tests/test_integration.py` verifies end-to-end roundtrip: seed → save → load → Elo update → save → reload → data matches
5. **Persistence guarantee:** All writes use tempfile + os.replace + os.fsync (no partial/corrupt files)
...
# Run all tests (Elo + persistence + integration)
...
| Match recording CLI (`--record-match`) | CLI Phase (later) | Programmatic integration test covers the logic |
...
| 2 | 02 | Elo engine, state persistence (atomic writes), programmatic integration test |
